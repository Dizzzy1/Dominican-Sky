...
function processPayment() {
    ...
    // Crear reserva en el MISMO formato que espera index.html
    const reservation = {
        id: nextId,
    ...
    if (currentUser && currentUser.type === 'corporate') {
        ...
        if (userIndex !== -1) {
            if (flightData.discount > 0) {
                savedUsers[userIndex].miles -= 1000; // Quitar millas usadas
            }
            savedUsers[userIndex].miles += 500; // Sumar millas ganadas
    ...
    console.log('📋 Total reservas:', existingReservations.length);
